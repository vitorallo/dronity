...
/*my custom options*/
char *victim_dev = NULL;
char *ioctlgroup = NULL;
char *excludecmd = NULL;
bool fixed_arg;
char *fixed_arg_type;
...
static void usage(void)
{
	...
	outputerr(" --device,-f: path to victim dev.\n");
	outputerr(" --ioctlgroup,-i: select specific ioctl cmd group.\n");
	outputerr(" --excludecmd,-e: exclude specifc ioctl command.\n");
	outputerr(" --fixed arg,-A: fixed arg for ioctl calls.\n");
	outputerr("\n");
...
static const char paramstr[] = "a:b:c:C:dDg:hIl:LN:mP:E:pqr:s:T:SV:vx:X:f:i:e:A:";
...
static const struct option longopts[] = {
	...
	/* support for my custom commands */
	{ "device", required_argument, NULL, 'f' },
	{ "ioctlgroup", required_argument, NULL, 'i' },
	{ "excludecmd", required_argument, NULL, 'e' },
	{ "fixedarg", optional_argument, NULL, 'A' },
	{ NULL, 0, NULL, 0 } };
...
void parse_args(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, paramstr, longopts, &opt_index)) != -1) {
		...
		/*handling my parameters*/
		case 'f':
			if (victim_dev == NULL) {
				victim_dev = strdup(optarg);
				victim_path = strdup(optarg);
				break;
				} else {
					outputstd("Sorry, only one dev path to define!\n");
					exit(EXIT_FAILURE);
				}
			break;
		...
		case 'i':
			if (ioctlgroup == NULL) {
				ioctlgroup = strdup(optarg);
				break;
			} else {
				outputstd("Only one group at the time!\n");
				exit(EXIT_FAILURE);
			}

		case 'e':
			if (excludecmd == NULL) {
				excludecmd = strdup(optarg);
				break;
			} else {
				outputstd("Only one cmd (command) at the time!\n");
				exit(EXIT_FAILURE);
			}
		...
		case 'A':
			fixed_arg = TRUE;
			if (fixed_arg_type == NULL) {
				if (strdup(optarg)!=NULL)
					fixed_arg_type = strdup(optarg);
				else
					strcpy (fixed_arg_type,"deadbeaf");
				break;
				} else {
					outputstd("Specify ONLY one fixed type or nothing!\n");
					exit(EXIT_FAILURE);
				}
			break;
