...
void inet_rand_socket(struct socket_triplet *st)
{
	unsigned long ipprotos[] = {
		...
		IPPROTO_IDP, IPPROTO_DCCP, IPPROTO_IPV6,
		IPPROTO_RSVP, IPPROTO_GRE, IPPROTO_ESP, IPPROTO_AH,
	        IPPROTO_BEETPH, IPPROTO_PIM,
		IPPROTO_COMP, IPPROTO_SCTP, IPPROTO_RAW,
	};
...
void ip_setsockopt(struct sockopt *so)
{
	...
	case IP_DROP_MEMBERSHIP:
		...
		/* the struct is defined differently, imr_multiaddr is not a struct but a __be32 (bitwise)
		 * need to invent something to assign properly
		 */
	...
	case IP_DROP_SOURCE_MEMBERSHIP:
		...
		/* need to find a way, multiaddr is not a struct but a bitwise __be32
		 * trying a simple cast, need to investigate this better
		 */
		ms->imr_multiaddr = (u32)mcaddr;
		ms->imr_interface = (u32)random_ipv4_address();
		ms->imr_sourceaddr = (u32)random_ipv4_address();
