...
//FIXME: call external victim dev, this shoud go into .h file
...
void setup_fd_providers(void)
{
	...
	if (victim_dev==NULL) {
		//register all fd providers otherwise only go with dev file_fd_providers
		add_to_prov_list(&socket_fd_provider);
		add_to_prov_list(&pipes_fd_provider);
		/*disabling anyhow fd perf under android+bionic it does not make sense
		...
		//add_to_prov_list(&timerfd_fd_provider); this provider has been commented for now!
		add_to_prov_list(&testfile_fd_provider);
		add_to_prov_list(&memfd_fd_provider);
	} else {
		output(0,"Victim device specified: %s", victim_dev);
		add_to_prov_list(&file_fd_provider);
	}
